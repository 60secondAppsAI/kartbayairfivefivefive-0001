...
const sortBy = ref('amenityId');
const sortOrder = ref('desc');

const shouldMaintainFocus = ref(false);

const filteredAmenitys = computed(() => {
  return amenitys.value;
});
...
const loadAmenitys = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/amenity/amenitys?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
    ...
    amenitys.value = data.amenitys || [];
    totalElements.value = data.totalElements || 0;
    totalPages.value = Math.ceil(totalElements.value / pageSize.value);
    currentPage.value = page;
  } catch (err: any) {
    console.error('Get amenitys error:', err);
    error.value = err.message || 'Failed to load amenitys';
  }
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadAmenitys(currentPage.value + 1);
  }
};
...
const getAmenityIcon = (amenity: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[amenity.amenityId % icons.length];
};

const viewAmenityDetails = (amenity: any) => {
  router.push({ name: 'AmenityDetail', params: { id: amenity.amenityId } });
};
...
<template>
  <div class="amenitys-page">
    <AppHeader />
    
    <header class="amenitys-header">
      <div class="header-row">
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              <h1 class="page-title">Amenitys</h1>
              <div class="amenitys-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="amenitys-content">
      <!-- Loading State -->
      <div v-if="isLoading" class="loading-state">
        <div class="loading-spinner-large"></div>
        <p class="loading-text">Loading amenitys...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="error-state">
        <div class="error-icon">⚠️</div>
        <h3 class="error-title">Failed to Load Amenitys</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadAmenitys()" class="retry-button">
          Try Again
        </button>
      </div>

      <!-- Empty State -->
      <div v-else-if="filteredAmenitys.length === 0" class="empty-state">
        <div class="empty-icon">👥</div>
        <h3 class="empty-title">No Amenitys Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No amenitys match your search criteria.' : 'No amenitys found in the system.' }}
        </p>
      </div>

      <!-- Amenitys Grid -->
      <div v-else-if="viewMode === 'cards'" class="amenitys-grid">
        <div
          v-for="amenity in filteredAmenitys"
        ...
        >
          ...
          <div class="amenity-info">
            <h3 class="amenity-name">{{ amenity.name || 'Unknown Amenity' }}</h3>
            <p class="amenity-description">
              Amenity ID: {{ amenity.amenityId }}
            </p>
            
            <div class="amenity-metadata">

              <div class="metadata-item">
                <span class="metadata-label">AmenityId:</span>
                <span class="metadata-value">{{ amenity.amenityId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Name:</span>
                <span class="metadata-value">{{ amenity.name || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Description:</span>
                <span class="metadata-value">{{ amenity.description || 'N/A' }}</span>
              </div>
      ...
      <!-- Amenitys Table -->
      <div v-else-if="viewMode === 'table'" class="amenitys-table-container">
        <table class="amenitys-table">
          <thead>
            <tr>

              <th @click="handleSort('amenityId')" class="sortable-header">
                AmenityId {{ getSortIcon('amenityId') }}
              </th>
          ...
          <tbody>
            <tr v-for="amenity in filteredAmenitys" :key="amenity.amenityId" class="amenity-row">
              
              
                      <td class="amenity-id-cell" @click="viewAmenityDetails(amenity)">{{ amenity.amenityId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ amenity.name }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ amenity.description }}</span>
                </div>
              </td>
...
.amenitys-table th:first-child,
.amenitys-table td:first-child {
  width: 140px;
  min-width: 140px;
  max-width: 140px;
}
