...
const loadImageDetails = async () => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/image/${imageId.value}`);
  ...
  <div class="image-detail-page">
    ...
    <main class="detail-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <button @click="loadImageDetails" class="retry-button">
          Try Again
        </button>
      </div>
      ...
      <div v-else-if="imageData" class="image-details">
        <div class="image-header-section">
          <div class="image-icon-large">{{ getImageIcon(imageData) }}</div>
          <div class="image-header-info">
            <h1 class="image-title">{{ imageData.name || 'Unknown Image' }}</h1>
            <p class="image-subtitle">Image ID: {{ imageData.imageId }}</p>
          </div>
        </div>
            ...
            <div class="tab-content">
                ...
                <div class="details-section">
                  ...
                  <div class="info-grid">
                    <div class="info-item">
                      ...
                      <span class="info-value">{{ imageData.imageId }}</span>
                    </div>

                    <div class="info-item">
                      <span class="info-label">ImageId</span>
                      <span class="info-value">{{ imageData.imageId || 'N/A' }}</span>
                    </div>
                    <div class="info-item">
                      <span class="info-label">URL</span>
                      <span class="info-value">{{ imageData.uRL || 'N/A' }}</span>
                    </div>
                    <div class="info-item">
                      <span class="info-label">Description</span>
                      <span class="info-value">{{ imageData.description || 'N/A' }}</span>
                    </div>
              ...
              <div v-else-if="activeTab === 'Activity'" class="tab-panel">
                <div class="empty-tab-state">
                  ...
                  <h3>Activity Log</h3>
                  <p>Image activity and history will be available here.</p>
                </div>
              </div>
            </div>
          </div>
