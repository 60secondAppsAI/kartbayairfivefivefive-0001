...
const loadImages = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/image/images?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const getImageIcon = (image: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[image.imageId % icons.length];
};

const viewImageDetails = (image: any) => {
  router.push({ name: 'ImageDetail', params: { id: image.imageId } });
};
  ...
  <div class="images-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="images-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="images-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <h3 class="error-title">Failed to Load Images</h3>
      ...
      <div v-else-if="filteredImages.length === 0" class="empty-state">
        ...
        <p class="empty-message">
          {{ searchQuery ? 'No images match your search criteria.' : 'No images found in the system.' }}
        </p>
      </div>

      <!-- Images Grid -->
      <div v-else-if="viewMode === 'cards'" class="images-grid">
        <div
        ...
        >
          ...
          <div class="image-info">
            <h3 class="image-name">{{ image.name || 'Unknown Image' }}</h3>
            ...
            <div class="image-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">ImageId:</span>
                <span class="metadata-value">{{ image.imageId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">URL:</span>
                <span class="metadata-value">{{ image.uRL || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Description:</span>
                <span class="metadata-value">{{ image.description || 'N/A' }}</span>
              </div>
      ...
      <!-- Images Table -->
      <div v-else-if="viewMode === 'table'" class="images-table-container">
        <table class="images-table">
          <thead>
            <tr>

              <th @click="handleSort('imageId')" class="sortable-header">
                ImageId {{ getSortIcon('imageId') }}
              </th>
              <th @click="handleSort('uRL')" class="sortable-header">
                URL {{ getSortIcon('uRL') }}
              </th>
          ...
          <tbody>
            <tr v-for="image in filteredImages" :key="image.imageId" class="image-row">
              
              
                      <td class="image-id-cell" @click="viewImageDetails(image)">{{ image.imageId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ image.uRL }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ image.description }}</span>
                </div>
              </td>
