...
const sortBy = ref('addressId');
const sortOrder = ref('desc');

const shouldMaintainFocus = ref(false);

const filteredAddresss = computed(() => {
...
const loadAddresss = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/address/addresss?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
    ...
    addresss.value = data.addresss || [];
    totalElements.value = data.totalElements || 0;
    totalPages.value = Math.ceil(totalElements.value / pageSize.value);
    currentPage.value = page;
  } catch (err: any) {
    console.error('Get addresss error:', err);
    error.value = err.message || 'Failed to load addresss';
  }
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadAddresss(currentPage.value + 1);
  }
};
...
const getAddressIcon = (address: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[address.addressId % icons.length];
};

const viewAddressDetails = (address: any) => {
  router.push({ name: 'AddressDetail', params: { id: address.addressId } });
};
...
<template>
  <div class="addresss-page">
    <AppHeader />
    
    <header class="addresss-header">
      <div class="header-row">
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              <h1 class="page-title">Addresss</h1>
              <div class="addresss-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="addresss-content">
      <!-- Loading State -->
      <div v-if="isLoading" class="loading-state">
        <div class="loading-spinner-large"></div>
        <p class="loading-text">Loading addresss...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="error-state">
        <div class="error-icon">⚠️</div>
        <h3 class="error-title">Failed to Load Addresss</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadAddresss()" class="retry-button">
          Try Again
        </button>
      </div>

      <!-- Empty State -->
      <div v-else-if="filteredAddresss.length === 0" class="empty-state">
        <div class="empty-icon">👥</div>
        <h3 class="empty-title">No Addresss Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No addresss match your search criteria.' : 'No addresss found in the system.' }}
        </p>
      </div>

      <!-- Addresss Grid -->
      <div v-else-if="viewMode === 'cards'" class="addresss-grid">
        <div
          v-for="address in filteredAddresss"
        ...
        >
          ...
          <div class="address-info">
            <h3 class="address-name">{{ address.name || 'Unknown Address' }}</h3>
            ...
            <div class="address-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">AddressId:</span>
                <span class="metadata-value">{{ address.addressId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Street:</span>
                <span class="metadata-value">{{ address.street || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">City:</span>
                <span class="metadata-value">{{ address.city || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">State:</span>
                <span class="metadata-value">{{ address.state || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">ZipCode:</span>
                <span class="metadata-value">{{ address.zipCode || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Country:</span>
                <span class="metadata-value">{{ address.country || 'N/A' }}</span>
              </div>
      ...
      <!-- Addresss Table -->
      <div v-else-if="viewMode === 'table'" class="addresss-table-container">
        <table class="addresss-table">
          <thead>
            <tr>

              <th @click="handleSort('addressId')" class="sortable-header">
                AddressId {{ getSortIcon('addressId') }}
              </th>
              <th @click="handleSort('street')" class="sortable-header">
                Street {{ getSortIcon('street') }}
              </th>
              <th @click="handleSort('city')" class="sortable-header">
                City {{ getSortIcon('city') }}
              </th>
              <th @click="handleSort('state')" class="sortable-header">
                State {{ getSortIcon('state') }}
              </th>
              <th @click="handleSort('zipCode')" class="sortable-header">
                ZipCode {{ getSortIcon('zipCode') }}
              </th>
              <th @click="handleSort('country')" class="sortable-header">
          ...
          <tbody>
            <tr v-for="address in filteredAddresss" :key="address.addressId" class="address-row">
              
              
                      <td class="address-id-cell" @click="viewAddressDetails(address)">{{ address.addressId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ address.street }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ address.city }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ address.state }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ address.zipCode }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ address.country }}</span>
                </div>
              </td>
...
.addresss-table th:first-child,
.addresss-table td:first-child {
  width: 140px;
  min-width: 140px;
  max-width: 140px;
}
