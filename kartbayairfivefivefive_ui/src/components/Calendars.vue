...
const loadCalendars = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/calendar/calendars?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
    ...
    error.value = err.message || 'Failed to load calendars';
  }
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadCalendars(currentPage.value + 1);
  }
};
...
const getCalendarIcon = (calendar: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[calendar.calendarId % icons.length];
};

const viewCalendarDetails = (calendar: any) => {
  router.push({ name: 'CalendarDetail', params: { id: calendar.calendarId } });
};
  ...
  <div class="calendars-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="calendars-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="calendars-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <h3 class="error-title">Failed to Load Calendars</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadCalendars()" class="retry-button">
          Try Again
        </button>
      </div>

      <!-- Empty State -->
      <div v-else-if="filteredCalendars.length === 0" class="empty-state">
        <div class="empty-icon">👥</div>
        <h3 class="empty-title">No Calendars Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No calendars match your search criteria.' : 'No calendars found in the system.' }}
        </p>
      </div>

      <!-- Calendars Grid -->
      <div v-else-if="viewMode === 'cards'" class="calendars-grid">
        <div
          v-for="calendar in filteredCalendars"
          :key="calendar.calendarId"
          class="calendar-card"
          @click="viewCalendarDetails(calendar)"
        >
          ...
          <div class="calendar-info">
            <h3 class="calendar-name">{{ calendar.name || 'Unknown Calendar' }}</h3>
            ...
            <div class="calendar-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">CalendarId:</span>
                <span class="metadata-value">{{ calendar.calendarId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Date:</span>
                <span class="metadata-value">{{ calendar.date || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Available:</span>
                <span class="metadata-value">{{ calendar.available || 'N/A' }}</span>
              </div>
      ...
      <!-- Calendars Table -->
      <div v-else-if="viewMode === 'table'" class="calendars-table-container">
        <table class="calendars-table">
          <thead>
            <tr>

              <th @click="handleSort('calendarId')" class="sortable-header">
                CalendarId {{ getSortIcon('calendarId') }}
              </th>
              <th @click="handleSort('date')" class="sortable-header">
                Date {{ getSortIcon('date') }}
              </th>
              <th @click="handleSort('available')" class="sortable-header">
          ...
          <tbody>
            <tr v-for="calendar in filteredCalendars" :key="calendar.calendarId" class="calendar-row">
              
              
                      <td class="calendar-id-cell" @click="viewCalendarDetails(calendar)">{{ calendar.calendarId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ calendar.date }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ calendar.available }}</span>
                </div>
              </td>
...
.calendars-table th:first-child,
.calendars-table td:first-child {
  width: 140px;
  min-width: 140px;
  max-width: 140px;
}
