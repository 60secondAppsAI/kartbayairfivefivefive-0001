...
const loadInsurances = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/insurance/insurances?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
    ...
    error.value = err.message || 'Failed to load insurances';
  }
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadInsurances(currentPage.value + 1);
  }
};
...
const getInsuranceIcon = (insurance: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[insurance.insuranceId % icons.length];
};

const viewInsuranceDetails = (insurance: any) => {
  router.push({ name: 'InsuranceDetail', params: { id: insurance.insuranceId } });
};
...
<template>
  <div class="insurances-page">
    ...
    <header class="insurances-header">
      <div class="header-row">
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              <h1 class="page-title">Insurances</h1>
              <div class="insurances-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="insurances-content">
      <!-- Loading State -->
      <div v-if="isLoading" class="loading-state">
        <div class="loading-spinner-large"></div>
        <p class="loading-text">Loading insurances...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="error-state">
        <div class="error-icon">⚠️</div>
        <h3 class="error-title">Failed to Load Insurances</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadInsurances()" class="retry-button">
          Try Again
        </button>
      </div>

      <!-- Empty State -->
      <div v-else-if="filteredInsurances.length === 0" class="empty-state">
        <div class="empty-icon">👥</div>
        <h3 class="empty-title">No Insurances Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No insurances match your search criteria.' : 'No insurances found in the system.' }}
        </p>
      </div>

      <!-- Insurances Grid -->
      <div v-else-if="viewMode === 'cards'" class="insurances-grid">
        <div
          v-for="insurance in filteredInsurances"
          :key="insurance.insuranceId"
          class="insurance-card"
          @click="viewInsuranceDetails(insurance)"
        >
          <div class="insurance-header">
            <div class="insurance-icon">
              {{ getInsuranceIcon(insurance) }}
            </div>
          </div>

          <div class="insurance-info">
            <h3 class="insurance-name">{{ insurance.name || 'Unknown Insurance' }}</h3>
            <p class="insurance-description">
              Insurance ID: {{ insurance.insuranceId }}
            </p>
            
            <div class="insurance-metadata">

              <div class="metadata-item">
                <span class="metadata-label">InsuranceId:</span>
                <span class="metadata-value">{{ insurance.insuranceId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">CoverageAmount:</span>
                <span class="metadata-value">{{ insurance.coverageAmount || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Provider:</span>
                <span class="metadata-value">{{ insurance.provider || 'N/A' }}</span>
              </div>
      ...
      <!-- Insurances Table -->
      <div v-else-if="viewMode === 'table'" class="insurances-table-container">
        <table class="insurances-table">
          <thead>
            <tr>

              <th @click="handleSort('insuranceId')" class="sortable-header">
                InsuranceId {{ getSortIcon('insuranceId') }}
              </th>
              <th @click="handleSort('coverageAmount')" class="sortable-header">
                CoverageAmount {{ getSortIcon('coverageAmount') }}
              </th>
              <th @click="handleSort('provider')" class="sortable-header">
          ...
          <tbody>
            <tr v-for="insurance in filteredInsurances" :key="insurance.insuranceId" class="insurance-row">
              
              
                      <td class="insurance-id-cell" @click="viewInsuranceDetails(insurance)">{{ insurance.insuranceId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ insurance.coverageAmount }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ insurance.provider }}</span>
                </div>
              </td>
...
.insurances-table th:first-child,
.insurances-table td:first-child {
  width: 140px;
  min-width: 140px;
  max-width: 140px;
}
