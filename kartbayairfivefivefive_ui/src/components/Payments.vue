...
const loadPayments = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/payment/payments?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadPayments(currentPage.value + 1);
  }
};
...
const getPaymentIcon = (payment: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[payment.paymentId % icons.length];
};

const viewPaymentDetails = (payment: any) => {
  router.push({ name: 'PaymentDetail', params: { id: payment.paymentId } });
};
  ...
  <div class="payments-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="payments-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="payments-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <h3 class="error-title">Failed to Load Payments</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadPayments()" class="retry-button">
          Try Again
        </button>
      </div>
      ...
      <div v-else-if="filteredPayments.length === 0" class="empty-state">
        ...
        <p class="empty-message">
          {{ searchQuery ? 'No payments match your search criteria.' : 'No payments found in the system.' }}
        </p>
      </div>

      <!-- Payments Grid -->
      <div v-else-if="viewMode === 'cards'" class="payments-grid">
        <div
        ...
        >
          ...
          <div class="payment-info">
            <h3 class="payment-name">{{ payment.name || 'Unknown Payment' }}</h3>
            ...
            <div class="payment-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">PaymentId:</span>
                <span class="metadata-value">{{ payment.paymentId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Amount:</span>
                <span class="metadata-value">{{ payment.amount || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">PaymentDate:</span>
                <span class="metadata-value">{{ payment.paymentDate || 'N/A' }}</span>
              </div>
      ...
      <!-- Payments Table -->
      <div v-else-if="viewMode === 'table'" class="payments-table-container">
        <table class="payments-table">
          <thead>
            <tr>

              <th @click="handleSort('paymentId')" class="sortable-header">
                PaymentId {{ getSortIcon('paymentId') }}
              </th>
              <th @click="handleSort('amount')" class="sortable-header">
                Amount {{ getSortIcon('amount') }}
              </th>
              <th @click="handleSort('paymentDate')" class="sortable-header">
                PaymentDate {{ getSortIcon('paymentDate') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="payment in filteredPayments" :key="payment.paymentId" class="payment-row">
              
              
                      <td class="payment-id-cell" @click="viewPaymentDetails(payment)">{{ payment.paymentId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ payment.amount }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ payment.paymentDate }}</span>
                </div>
              </td>
