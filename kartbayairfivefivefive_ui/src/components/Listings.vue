...
const loadListings = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/listing/listings?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const getListingIcon = (listing: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[listing.listingId % icons.length];
};

const viewListingDetails = (listing: any) => {
  router.push({ name: 'ListingDetail', params: { id: listing.listingId } });
};
  ...
  <div class="listings-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="listings-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="listings-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <h3 class="error-title">Failed to Load Listings</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadListings()" class="retry-button">
          Try Again
        </button>
      </div>
      ...
      <div v-else-if="filteredListings.length === 0" class="empty-state">
        ...
        <p class="empty-message">
          {{ searchQuery ? 'No listings match your search criteria.' : 'No listings found in the system.' }}
        </p>
      </div>

      <!-- Listings Grid -->
      <div v-else-if="viewMode === 'cards'" class="listings-grid">
        <div
        ...
        >
          ...
          <div class="listing-info">
            <h3 class="listing-name">{{ listing.name || 'Unknown Listing' }}</h3>
            ...
            <div class="listing-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">ListingId:</span>
                <span class="metadata-value">{{ listing.listingId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Title:</span>
                <span class="metadata-value">{{ listing.title || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Description:</span>
                <span class="metadata-value">{{ listing.description || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Location:</span>
                <span class="metadata-value">{{ listing.location || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">PricePerNight:</span>
                <span class="metadata-value">{{ listing.pricePerNight || 'N/A' }}</span>
              </div>
      ...
      <!-- Listings Table -->
      <div v-else-if="viewMode === 'table'" class="listings-table-container">
        <table class="listings-table">
          <thead>
            <tr>

              <th @click="handleSort('listingId')" class="sortable-header">
                ListingId {{ getSortIcon('listingId') }}
              </th>
              ...
              <th @click="handleSort('location')" class="sortable-header">
                Location {{ getSortIcon('location') }}
              </th>
              <th @click="handleSort('pricePerNight')" class="sortable-header">
                PricePerNight {{ getSortIcon('pricePerNight') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="listing in filteredListings" :key="listing.listingId" class="listing-row">
              
              
                      <td class="listing-id-cell" @click="viewListingDetails(listing)">{{ listing.listingId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ listing.title }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ listing.description }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ listing.location }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ listing.pricePerNight }}</span>
                </div>
              </td>
