...
const loadPromotions = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/promotion/promotions?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadPromotions(currentPage.value + 1);
  }
};
...
const getPromotionIcon = (promotion: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[promotion.promotionId % icons.length];
};

const viewPromotionDetails = (promotion: any) => {
  router.push({ name: 'PromotionDetail', params: { id: promotion.promotionId } });
};
  ...
  <div class="promotions-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="promotions-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="promotions-content">
      ...
      <div v-if="isLoading" class="loading-state">
        ...
        <p class="loading-text">Loading promotions...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="error-state">
        <div class="error-icon">⚠️</div>
        <h3 class="error-title">Failed to Load Promotions</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadPromotions()" class="retry-button">
          Try Again
        </button>
      </div>

      <!-- Empty State -->
      <div v-else-if="filteredPromotions.length === 0" class="empty-state">
        <div class="empty-icon">👥</div>
        <h3 class="empty-title">No Promotions Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No promotions match your search criteria.' : 'No promotions found in the system.' }}
        </p>
      </div>

      <!-- Promotions Grid -->
      <div v-else-if="viewMode === 'cards'" class="promotions-grid">
        <div
        ...
        >
          ...
          <div class="promotion-info">
            <h3 class="promotion-name">{{ promotion.name || 'Unknown Promotion' }}</h3>
            <p class="promotion-description">
              Promotion ID: {{ promotion.promotionId }}
            </p>
            
            <div class="promotion-metadata">

              <div class="metadata-item">
                <span class="metadata-label">PromotionId:</span>
                <span class="metadata-value">{{ promotion.promotionId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Details:</span>
                <span class="metadata-value">{{ promotion.details || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">StartDate:</span>
                <span class="metadata-value">{{ promotion.startDate || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">EndDate:</span>
                <span class="metadata-value">{{ promotion.endDate || 'N/A' }}</span>
              </div>
      ...
      <!-- Promotions Table -->
      <div v-else-if="viewMode === 'table'" class="promotions-table-container">
        <table class="promotions-table">
          <thead>
            <tr>

              <th @click="handleSort('promotionId')" class="sortable-header">
                PromotionId {{ getSortIcon('promotionId') }}
              </th>
              <th @click="handleSort('details')" class="sortable-header">
                Details {{ getSortIcon('details') }}
              </th>
              <th @click="handleSort('startDate')" class="sortable-header">
                StartDate {{ getSortIcon('startDate') }}
              </th>
              <th @click="handleSort('endDate')" class="sortable-header">
                EndDate {{ getSortIcon('endDate') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="promotion in filteredPromotions" :key="promotion.promotionId" class="promotion-row">
              
              
                      <td class="promotion-id-cell" @click="viewPromotionDetails(promotion)">{{ promotion.promotionId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ promotion.details }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ promotion.startDate }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ promotion.endDate }}</span>
                </div>
              </td>
