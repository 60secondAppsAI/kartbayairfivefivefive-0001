...
const loadBookings = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/booking/bookings?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadBookings(currentPage.value + 1);
  }
};
...
const viewBookingDetails = (booking: any) => {
  router.push({ name: 'BookingDetail', params: { id: booking.bookingId } });
};
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="bookings-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="bookings-content">
      ...
      <div v-else-if="filteredBookings.length === 0" class="empty-state">
        ...
        <p class="empty-message">
          {{ searchQuery ? 'No bookings match your search criteria.' : 'No bookings found in the system.' }}
        </p>
      </div>

      <!-- Bookings Grid -->
      <div v-else-if="viewMode === 'cards'" class="bookings-grid">
        <div
        ...
        >
          ...
          <div class="booking-info">
            <h3 class="booking-name">{{ booking.name || 'Unknown Booking' }}</h3>
            ...
            <div class="booking-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">BookingId:</span>
                <span class="metadata-value">{{ booking.bookingId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">StartDate:</span>
                <span class="metadata-value">{{ booking.startDate || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">EndDate:</span>
                <span class="metadata-value">{{ booking.endDate || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">TotalCost:</span>
                <span class="metadata-value">{{ booking.totalCost || 'N/A' }}</span>
              </div>
      ...
      <!-- Bookings Table -->
      <div v-else-if="viewMode === 'table'" class="bookings-table-container">
        <table class="bookings-table">
          <thead>
            <tr>

              <th @click="handleSort('bookingId')" class="sortable-header">
                BookingId {{ getSortIcon('bookingId') }}
              </th>
              ...
              <th @click="handleSort('totalCost')" class="sortable-header">
                TotalCost {{ getSortIcon('totalCost') }}
              </th>
          ...
          <tbody>
            <tr v-for="booking in filteredBookings" :key="booking.bookingId" class="booking-row">
                      ...
                      <td class="booking-id-cell" @click="viewBookingDetails(booking)">{{ booking.bookingId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ booking.startDate }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ booking.endDate }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ booking.totalCost }}</span>
                </div>
              </td>
