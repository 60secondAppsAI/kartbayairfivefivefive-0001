...
const loadHosts = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/host/hosts?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const getHostIcon = (host: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[host.hostId % icons.length];
};

const viewHostDetails = (host: any) => {
  router.push({ name: 'HostDetail', params: { id: host.hostId } });
};
...
<template>
  <div class="hosts-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="hosts-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="hosts-content">
      <!-- Loading State -->
      <div v-if="isLoading" class="loading-state">
        <div class="loading-spinner-large"></div>
        <p class="loading-text">Loading hosts...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="error-state">
        <div class="error-icon">⚠️</div>
        <h3 class="error-title">Failed to Load Hosts</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadHosts()" class="retry-button">
          Try Again
        </button>
      </div>
      ...
      <div v-else-if="filteredHosts.length === 0" class="empty-state">
        ...
        <h3 class="empty-title">No Hosts Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No hosts match your search criteria.' : 'No hosts found in the system.' }}
        </p>
      </div>

      <!-- Hosts Grid -->
      <div v-else-if="viewMode === 'cards'" class="hosts-grid">
        <div
          ...
          <div class="host-info">
            ...
            <div class="host-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">HostId:</span>
                <span class="metadata-value">{{ host.hostId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Name:</span>
                <span class="metadata-value">{{ host.name || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Email:</span>
                <span class="metadata-value">{{ host.email || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">PhoneNumber:</span>
                <span class="metadata-value">{{ host.phoneNumber || 'N/A' }}</span>
              </div>
      ...
      <!-- Hosts Table -->
      <div v-else-if="viewMode === 'table'" class="hosts-table-container">
        <table class="hosts-table">
          <thead>
            <tr>

              <th @click="handleSort('hostId')" class="sortable-header">
                HostId {{ getSortIcon('hostId') }}
              </th>
              ...
              <th @click="handleSort('phoneNumber')" class="sortable-header">
                PhoneNumber {{ getSortIcon('phoneNumber') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="host in filteredHosts" :key="host.hostId" class="host-row">
              
              
                      <td class="host-id-cell" @click="viewHostDetails(host)">{{ host.hostId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ host.name }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ host.email }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ host.phoneNumber }}</span>
                </div>
              </td>
