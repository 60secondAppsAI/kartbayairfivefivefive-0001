...
const loadReviews = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/review/reviews?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
...
const getReviewIcon = (review: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[review.reviewId % icons.length];
};

const viewReviewDetails = (review: any) => {
  router.push({ name: 'ReviewDetail', params: { id: review.reviewId } });
};
  ...
  <div class="reviews-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="reviews-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="reviews-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <h3 class="error-title">Failed to Load Reviews</h3>
      ...
      <div v-else-if="filteredReviews.length === 0" class="empty-state">
        ...
        <p class="empty-message">
          {{ searchQuery ? 'No reviews match your search criteria.' : 'No reviews found in the system.' }}
        </p>
      </div>

      <!-- Reviews Grid -->
      <div v-else-if="viewMode === 'cards'" class="reviews-grid">
        <div
        ...
        >
          ...
          <div class="review-info">
            <h3 class="review-name">{{ review.name || 'Unknown Review' }}</h3>
            ...
            <div class="review-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">ReviewId:</span>
                <span class="metadata-value">{{ review.reviewId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Rating:</span>
                <span class="metadata-value">{{ review.rating || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Comment:</span>
                <span class="metadata-value">{{ review.comment || 'N/A' }}</span>
              </div>
      ...
      <!-- Reviews Table -->
      <div v-else-if="viewMode === 'table'" class="reviews-table-container">
        <table class="reviews-table">
          <thead>
            <tr>

              <th @click="handleSort('reviewId')" class="sortable-header">
                ReviewId {{ getSortIcon('reviewId') }}
              </th>
          ...
          <tbody>
            <tr v-for="review in filteredReviews" :key="review.reviewId" class="review-row">
              
              
                      <td class="review-id-cell" @click="viewReviewDetails(review)">{{ review.reviewId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ review.rating }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ review.comment }}</span>
                </div>
              </td>
